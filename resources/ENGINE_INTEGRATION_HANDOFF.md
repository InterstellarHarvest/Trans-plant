...
- **Real TARGETING**: FIRE LASER → crosshair follows the mouse (cursor:none), click within 220px of the enemy ship to aim; beam launches from the viewport's bottom-center to the click point. CANCEL button offered. **Engine addition over the demo: 1.6s auto-fire fallback at ship center** so keyboard players and the fuzz harness can never wedge in TARGETING (fuzz clicks buttons only, and the aim click is on the scene).
